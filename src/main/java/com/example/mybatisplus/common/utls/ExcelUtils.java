...
public class ExcelUtils {
    ...
    public static void exportMembers(HttpServletResponse httpServletResponse, List<Member> members) throws IOException {
        //声明一个表格对象
        HSSFWorkbook hwb = new HSSFWorkbook();
        //声明一个sheet并命名
        HSSFSheet sheet = hwb.createSheet("会员信息");
        //给sheet名称一个长度
        sheet.setDefaultColumnWidth((short) 10);
        //会员号
        ...
        for (short i = 0; i < members.size(); i++) {
            ...
            cell = row.createCell((short) 3);
            cell.setCellValue(members.get(i).getPoint());
            cell.setCellStyle(commStyle);
